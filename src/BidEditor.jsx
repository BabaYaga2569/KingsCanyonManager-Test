...
export default function BidEditor() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        ...
        // Preserve image rendering fields (not edited in BidEditor, just preserved)
        hasAiConceptRenderingImage: Boolean(bid.aiConceptRenderingImageUrl),
        aiConceptRenderingImageUrl: bid.aiConceptRenderingImageUrl || null,
        aiConceptRenderingPrompt: bid.aiConceptRenderingPrompt || null,
        aiConceptRenderingStyle: bid.aiConceptRenderingStyle || null,
        aiConceptRenderingModel: bid.aiConceptRenderingModel || null,
        aiConceptRenderingGeneratedAt: bid.aiConceptRenderingGeneratedAt || null,
  ...
  const buildBidPDF = async () => {
    ...
    if (bid.hasAiConceptRendering && bid.aiConceptRendering) {
      ...
      // Embed AI rendering image if available
      if (bid.aiConceptRenderingImageUrl) {
        try {
          const renderImgDataUrl = await new Promise((resolve, reject) => {
            ...
            imgEl.onload = () => {
              try {
                ...
                c.width = imgEl.width * scale;
                c.height = imgEl.height * scale;
                const ctx = c.getContext("2d");
            ...
            imgEl.src = bid.aiConceptRenderingImageUrl;
          });

          const imgPdfW = W - 80;
          const imgPdfH = Math.round(imgPdfW / (16 / 9));
          ...
          docPDF.addImage(renderImgDataUrl, "JPEG", 40, y, imgPdfW, imgPdfH);
          y += imgPdfH + 12;
          ...
          docPDF.text(`AI Concept Rendering · ${bid.aiConceptRenderingStyle || cr?.stylePreset || ""} · Generated by Kings Canyon Landscaping`, 40, y);
          docPDF.setTextColor(0);
          y += 14;
        } catch (imgErr) {
          console.warn("Could not embed rendering image in PDF:", imgErr);
          docPDF.setFont("helvetica", "italic");
          docPDF.setFontSize(9);
          docPDF.setTextColor(100);
          docPDF.text("[AI landscape rendering image available — view online at the signing link]", 40, y);
          docPDF.setTextColor(0);
          y += 16;
        }
      }
  ...
  const handleGeneratePDF = async () => {
    try {
      const docPDF = await buildBidPDF();
      if (!docPDF) return;
          ...
          {(bid.hasAiConceptRenderingImage || bid.hasAiConceptRendering) && (
            <Box sx={{ mt: 2 }}>
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 2 }}>
                ...
                <Chip
                  label={bid.hasAiConceptRenderingImage ? "Image Generated" : "Text Concept Only"}
                  size="small"
                  color={bid.hasAiConceptRenderingImage ? "success" : "default"}
                  variant="outlined"
                />
              </Box>

              {/* Rendering Image - shown prominently */}
              {bid.aiConceptRenderingImageUrl ? (
                <Box sx={{ mb: 2 }}>
                  <Box
                    sx={{
                      display: "grid",
                      gridTemplateColumns: bid.aiConceptRenderingSourcePhotoUrl
                        ? { xs: "1fr", sm: "1fr 1fr" }
                        : "1fr",
                      gap: 2,
                      mb: 1.5,
                  ...
                  <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mt: 1 }}>
                    {bid.aiConceptRenderingStyle && (
                      <Chip label={bid.aiConceptRenderingStyle} size="small" color="primary" />
                    )}
                    {bid.aiConceptRenderingModel && (
                      <Chip label={bid.aiConceptRenderingModel} size="small" variant="outlined" />
                    )}
                    {bid.aiConceptRenderingGeneratedAt && (
                      <Typography variant="caption" color="text.secondary" sx={{ alignSelf: "center" }}>
                        Generated: {new Date(bid.aiConceptRenderingGeneratedAt).toLocaleString()}
                      </Typography>
                    )}
                  </Box>
                </Box>
                ...
                <Alert severity="warning" sx={{ mb: 2 }}>
                  No AI rendering image on this bid. Only text concept data is stored. Re-create the bid to generate a visual rendering.
                </Alert>
              )}

              {/* Editable text concept metadata */}
              {conceptEdits && (
                <Card
                  sx={{
              ...
              <Alert severity="info" sx={{ mb: 1 }}>
                <strong>Rendering Note:</strong> The AI rendering image and design details will appear on the client-facing signing page and PDF. Use Save Changes to persist any text edits above.
              </Alert>
            </Box>
          )}
